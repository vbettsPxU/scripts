...
const filePatterns = [
  './blocks/block-button-group/default/*.json'
]
...
function getHighestPropCount(parentObj, prop) {
  ...
  Object.keys(parentObj).forEach(key => {
    if (key.includes(prop)) {
      keyCount = parseInt(key.substr(key.lastIndexOf('_')).replace('_', ''), 10);
      if (keyCount > count) {
        count = keyCount;
      }
    }
...
function processSettings(content, translationPath, sectionKey, blockName = null) {
  ...
  const updatedSettings = content.map(setting => {
    ...
    if (setting.type === 'header') {
      let countHeaders = localSchema[sectionKey];

      if (blockName) {
        countHeaders = localSchema[sectionKey].blocks[blockName];
      }

      headerCount = getHighestPropCount(countHeaders, 'header');
      ...
      let countParagraphs = localSchema[sectionKey];

      if (blockName) {
        countParagraphs = localSchema[sectionKey].blocks[blockName];
      }
      paragraphCount = getHighestPropCount(countParagraphs, 'paragraph');
